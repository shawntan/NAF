...
class model(object):
    ...
    def train(self, epoch, total=10000):
        log_file = open('anneal_experiment_%d.log' % total, 'w')
        ...
        for e in range(epoch):
            for x in self.train_loader:
                ...
                rec, ent, xent = self.vae.loss(x, breakdown=True)
                RECS += rec.sum().data.cpu().numpy()
                ENTS += ent.sum().data.cpu().numpy()
                XENTS += xent.sum().data.cpu().numpy()
            ...
            if e % 1 == 0:
                self.vae.optim.swap()
                # loss_val = self.evaluate_vlb(self.valid_loader)[0]
                ...
                #        loss_val, loss_tst), weight
                valid_recon, valid_ent, valid_ce = \
                    self.evaluate_vlb(self.valid_loader)
                test_recon, test_ent, test_ce = \
                    self.evaluate_vlb(self.test_loader)
                loss_val = valid_recon + valid_ce - valid_ent
                log_entry = {
                    'train_recon': (RECS / float(counter))[0],
                    'valid_recon': valid_recon[0],
                    'test_recon': test_recon[0],
                    'train_ent': (ENTS / float(counter))[0],
                    'valid_ent': valid_ent[0],
                    'test_ent': test_ent[0],
                    'train_ce': (XENTS / float(counter))[0],
                    'valid_ce': valid_ce[0],
                    'test_ce': test_ce[0],
                }
                for k in log_entry:
                    log_entry[k] = float(log_entry[k])
    ...
    def evaluate_vlb(self, dataloader):
        total_recon_loss = 0
        total_entropy = 0
        ...
        for x in dataloader:
            ...
            if self.cuda:
                x = x.cuda()
            loss = self.vae.loss(x, breakdown=True)
            total_recon_loss += loss[0].sum()
            total_entropy += loss[1].sum()
            total_cross_entropy += loss[2].sum()
            c += loss[0].shape[0]
        return ((total_recon_loss / float(c)).data.cpu().numpy(),
                (total_entropy / float(c)).data.cpu().numpy(),
                (total_cross_entropy / float(c)).data.cpu().numpy())
